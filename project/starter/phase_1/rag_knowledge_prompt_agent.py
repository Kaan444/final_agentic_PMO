...
# Env setup (robust)
# -------------------------
load_dotenv()  # looks in current working directory by default
...
# Fewer chunks = fewer embedding calls = much faster
...
# RAG pipeline
# -------------------------
print("1) Chunking knowledge text...")
chunks = rag_agent.chunk_text(knowledge_text)
print(f"   ✅ Created {len(chunks)} chunks.")
print(f"   ✅ Chunks file: chunks-{rag_agent.unique_filename}")

print("2) Ensuring embeddings exist (cached per-run)...")
embeddings_path = Path(f"embeddings-{rag_agent.unique_filename}")

if embeddings_path.exists():
    print(f"   ✅ Using cached embeddings: {embeddings_path.name}")
else:
    print("   ⏳ Computing embeddings (first run can take a bit)...")
    df = rag_agent.calculate_embeddings()
    print(f"   ✅ Generated embeddings for {len(df)} chunks.")
    print(f"   ✅ Embeddings file: embeddings-{rag_agent.unique_filename}")

prompt = "What is the podcast that Clara hosts about?"
print("\n3) Prompt:")
print(f"   {prompt}")

print("\n4) Retrieving best chunk + generating answer...")
...
print(
    "\nExplanation: The agent chunks the knowledge text, computes embeddings for each chunk (cached to CSV), "
    "retrieves the most relevant chunk using cosine similarity to the prompt embedding, then answers using only "
    "the retrieved chunk as context."
)
